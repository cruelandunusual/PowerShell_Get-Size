...
By default `Get-Size` sorts its results by size in ascending order, so you can leave out the `-Ascending` switch.  
...
```
Leaving out filenames, variables or wildcards will default to showing the content of the current directory,  
the same way `Get-ChildItem` works, so the following are equivalent:  
